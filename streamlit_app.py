"""
💎 PREMIUM JEWELLERY SHOP AI DASHBOARD v3.3 (ENHANCED)
Complete AI + BI System for Indian Jewellery Retail
NEW: Send Return Offer working + Customer filters for pending
"""
...
def show_customers():
    ...
    with tab1:
        ...
        st.metric(f"Total Customers Found: {len(filtered)}")
    ...
    with tab4:
        ...
        st.metric(f"Customers with ₹{min_pending:,.0f}+ pending", len(pending_customers))
        st.metric(f"Total pending amount", f"₹{pending_customers['pending_amount'].sum():,.0f}")
...
# ============================================================================
# QUICK ACTIONS PAGE - WITH FIXED SEND RETURN OFFER
# ============================================================================
...
Shree Jewels""",
        ...
        # NEW WORKING SECTION FOR RETURN OFFER
        # Find dormant customers (no visit > 60 days)
        today = datetime.now().date()
...
Shree Jewels ✨""",
            ...
            if st.button(f"📤 Send Return Offers to {len(dormant_customers)} Customers", use_container_width=True):
                ...
                **Campaign Summary:**
                ...
                # Show sample of who received it
...
def main():
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("**v3.3** | AI-Powered Jewellery Retail Management ✅")
