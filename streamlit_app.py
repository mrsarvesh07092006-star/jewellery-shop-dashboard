# (START OF FILE) streamlit_app.v5.py
...
def get_accessible_pages(role):
    pages = {
        "Manager": [
            ...
            "💬 Smart Commands",
            "💬 Chatbot",
            "🔎 Integrated Chatbot"   # <-- new data-aware chatbot
        ],
        "Sales Staff": [
            "📊 Dashboard",
            "👥 Customers",
            ...
            "🤖 AI Assistant",
            "💬 Chatbot",
            "🔎 Integrated Chatbot"   # <-- staff can use integrated chatbot to fetch customers
        ],
        "Customer": [
            "💎 My Dashboard",
            "🛍️ My Purchases",
...
def staff_dashboard_page():
    ...
    with col1:
        if st.button("🔔 Show Pending Commissions", use_container_width=True):
            pending_df = pd.DataFrame.from_records([
                {"Staff Name": v["name"], "Position": v["position"], "Pending Amount": v["pending"]} for k, v in STAFF_MEMBERS.items()
            ])
    ...
    with col3:
        if st.button("📥 Export Sales (CSV)", use_container_width=True):
            # example export
            sales_example = pd.DataFrame({
                'Date': ['2025-12-10', '2025-12-09'],
                'Item': ['Gold Ring', 'Silver Bracelet'],
                'Amount': [45000, 8000]
            })
    ...
    # Staff view: recent sales / tasks
    st.subheader("📋 Today's Tasks & Recent Sales")
    sales_df = pd.DataFrame({
        'Date': ['2025-12-10', '2025-12-09', '2025-12-08'],
        'Item': ['Gold Ring', 'Silver Bracelet', 'Diamond Pendant'],
        'Amount': ['₹45,000', '₹8,000', '₹55,000'],
...
def integrated_chatbot_with_data_page():
    """
    Data-aware chatbot: simple keyword parsing to return customer info, purchases, pending payments,
    and export options. No external API required.
    """
    ...
    with col1:
        if st.button("🔍 Show customer summary"):
            # show the stored single customer summary
    ...
    with col4:
        if st.button("⬇️ Export customers CSV"):
            # Merge customers (example uses CUSTOMER_DATA and CUSTOMER_PURCHASES)
            cust = CUSTOMER_DATA.get("customer", {})
    ...
    if prompt:
        ...
        if q.startswith("show customer") or "show customer" in q:
            ...
            if not name:
                ...
                # match by name in stored customer name or in items
